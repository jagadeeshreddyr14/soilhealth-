...
# 
# remark_oc <- switch (avg_oc,
#   avg_oc < standard_oc[1], remark_oc <- remark.csv$Generic.Remarks[10],
#   avg_oc > standard_oc[2], remark_oc <- remark.csv$Generic.Remarks[12],
#   avg_oc , remark_oc <- remark.csv$Generic.Remarks[11]
# )
# 
# remark_k <- switch (avg_k,
#   avg_k < standard_k[1], remark_k <- remark.csv$Generic.Remarks[7],
#   avg_k > standard_k[2], remark_k <- remark.csv$Generic.Remarks[9],
#   avg_k , remark_k <- remark.csv$Generic.Remarks[8]
# )
# 
# remark_n <- switch (avg_n,
#   avg_n < standard_N[1], remark_n <- remark.csv$Generic.Remarks[1],
#   avg_n > standard_N[2], remark_n <- remark.csv$Generic.Remarks[3],
#   avg_n , remark_n <- remark.csv$Generic.Remarks[2]
# )
# 
# 
# remark_p <- switch (avg_p,
#   avg_p < standard_P[1], remark_p <- remark.csv$Generic.Remarks[4],
#   avg_p > standard_P[2], remark_p <- remark.csv$Generic.Remarks[6],
#   avg_p , remark_p <- remark.csv$Generic.Remarks[5]
# )
# 
# remark_ph<- switch (avg_ph,
#   avg_ph <= 5.5, remark_ph <- remark.csv$Generic.Remarks[13],
#   (avg_ph >=5.6& avg_ph <=6.5)  , remark_ph <- remark.csv$Generic.Remarks[14],
#   (avg_ph > 6.5 & avg_ph <=7.0) , remark_ph <- remark.csv$Generic.Remarks[15],
#   (avg_ph > 7.0 & avg_ph <= 7.5), remark_ph <- remark.csv$Generic.Remarks[16],
#   (avg_ph > 7.5 & avg_ph <= 8.5), remark_ph <- remark.csv$Generic.Remarks[17],
#   (avg_ph > 8.5), remark_ph <- remark.csv$Generic.Remarks[18]
# )
if (avg_ph <= 5.5){
...
\title{\textbf{Satyukt Analytics Pvt Ltd \\ Soil Analysis Report}}
\date{}
\maketitle
\centering
\centering
...
\begin{tblr}{
    ...
    vlines,
    row{1-4}={10ex}}
\centering Farm ID & \centering {`r in.filename`} \\
\centering Farm Name & \centering \\
\centering Date of Satellite pass & \centering {`r date`}\\
\centering Location & \centering {`r comb`} \\
\end{tblr}
\thispagestyle{fancy}
\fancyfoot[C]{117, 2nd Floor, Ashwin Arcade, Sanjay Nagar Main Road, Ashwath Nagar, Bangalore 560094 \\ Email: contact@satyukt.com, Contact No.: +91 6361763205 \\www.satyukt.com}
...
\vspace{0.5in}
\textbf{Disclaimer:
This is generated on the date when the land was barren}
