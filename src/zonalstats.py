...
def get_zonal_stats(farm_path, farm_id):
    
    tif_loc = f'../output/tif/{farm_id}'
    
    feat = get_py_geometry(farm_path)
    raster_images = glob(f"{tif_loc}/*")
    soil_nutrients = {}
    for raster_image in raster_images:

        name = os.path.splitext(os.path.basename(raster_image))[0]

        raster_namedate = name.split('_')[-1]
        raster_filename = ''.join(x for x in raster_namedate if x.isalpha())
        stats = zonal_stats(
            feat, raster_image, stats='median percentile_5 percentile_95', geojson_out=True, nodata=-999)
...
if __name__ == "__main__":
    ...
    for raster_image in raster_images:
        ...
        stats = zonal_stats(
            feature, raster_image, stats='median percentile_1', geojson_out=True, nodata=-999)
